                    ...
                    if sel.upper() == 'Y': # if yes
                        while True: # another loop
                            ...
                            if row > baserow:
                                ...
                                remrow = baserow - row
                                for x in range(remrow):
                                    field.remove(field[0])
                            elif row == baserow:
                                print(f'Row is already set to {row}')
                            if column > basecolumn:
                                newcol = column - basecolumn
                                for row in field:
                                    for x in range(newcol):
                                        row.append(row[0])
                            elif column < basecolumn:
                                remcol = basecolumn - column
                                for row in field:
                                    for x in range(remcol):
                                        row.remove(row[0])
                            elif column == basecolumn:
                                print(f'Column is already set to {column}')
